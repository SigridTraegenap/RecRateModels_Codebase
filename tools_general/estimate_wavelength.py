...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct 31 01:54:40 2024
...
def estimate_modularity(act_flat):
    ...
    plt.scatter(x=pos_min, y=np.ones_like(pos_min))
    # plt.plot(spectrum[:,:].T, color='C1')
    ...
    # print(np.diff(a))
    return spectrum[:,pos_max] - spectrum[:,pos_min]

def estimate_wavelength(act_flat):
    res_wl=eventAutocorrFitWavelength(act_flat, ~np.isnan(act_flat[0]),
                                          )
    autocorr,spectrum,distance = res_wl
    pos_min = np.argmax(-spectrum[:,(spectrum.shape[-1]//2):], axis=1)
